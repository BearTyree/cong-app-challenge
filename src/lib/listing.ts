...
export const CONDITIONS = [
  ...
  },
  {
    value: "gently-used",
    label: "Gently Used",
    description: "Used with care, minor wear",
  },
  { value: "used", label: "Used", description: "Normal wear and tear" },
  {
    value: "well-worn",
    label: "Well Worn",
    description: "Significant use but functional",
  },
] as const;
...
export const listingSchema = z.object({
  ...
  category: z
    .enum(CATEGORIES.map((c) => c.id) as [Category, ...Category[]])
    .describe("Please select a category"),

  condition: z
    .enum(CONDITIONS.map((c) => c.value) as [Condition, ...Condition[]])
  ...
  images: z
    ...
    .max(
      ...
      `Maximum ${FORM_LIMITS.maxImages} images allowed`
    )
    .refine(
  ...
  availabilityDays: z
    .array(z.enum(WEEKDAYS.map((d) => d.value) as [WeekDay, ...WeekDay[]]))
...
export const validateImageFile = (file: File): boolean => {
  return (
    file.size <= FORM_LIMITS.maxFileSize &&
    FORM_LIMITS.acceptedImageTypes.includes(file.type)
  );
};
